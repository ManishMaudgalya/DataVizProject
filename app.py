# =============================================================================
# app.py  —  Threat, Space & Control: Euro 2024 Analytics
#
# Fixes applied in this version
# ─────────────────────────────
# • Shot map: xG bubble sizes capped at 55px (was 400px → giant yellow circle)
# • Pass network: "Most linked pair" shown in full via st.markdown, not clipped metric
# • Pitch control: float→int cast in rasterizer (was crashing with TypeError)
# • xT: compute_threat_added now uses team_name not team_id → threat arrows appear
# • Team shape: side-by-side subplot when "both teams" is selected
# • Figure titles moved to st.subheader / st.markdown so they never clip
# • Pitch shapes all have layer="below" → markers always visible
# • Events Explorer: None → "N/A" for missing end coords / result
# =============================================================================
...
# ============================================================================
with st.sidebar:
    st.markdown("## ⚽ Euro 2024 Analytics")
    st.markdown("*Threat · Space · Control*")
    st.markdown("---")
...
with tab_shots:
    ...
    if shots.empty:
        ...
        # Summary table — clean up None result values
        summary = (
